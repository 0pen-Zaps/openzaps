...
export function ProfileDashboard(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const updateAccounts = (value: unknown): void => {
      ...
      try {
        next = first ? getAddress(first) : null;
      } catch {
        next = null;
      }
    ...
    const updateChain = (value: unknown): void => {
      if (!cancelled && typeof value === "string") setWalletChainId(Number.parseInt(value, 16));
    };
      ...
      <main className={styles.page} id="main">
        <section className={`container ${styles.connectHero}`}>
          <div>
            ...
            <p>
              Connect a wallet to read every confirmed capsule event attributed to it across v1.1, v3, and v3.1,
              then inspect, revoke, replace, or recover supported automations.
            </p>
          ...
          <div className={styles.connectPreview} aria-label="Profile capabilities">
            ...
            <p>No account is uploaded to an OpenZaps profile database. The connected address selects public chain logs and its relayed executor records.</p>
          </div>
        </section>
      </main>
    );
  ...
  const chainMismatch = walletChainId !== null && walletChainId !== ROBINHOOD_CHAIN_ID;
    ...
    <main className={styles.page} id="main">
      <section className={`container ${styles.profileHead}`}>
        ...
        <div className={styles.headActions}>
          ...
          <button className="btn btnGhost" disabled={loading || busy !== null} onClick={() => void refreshDashboard()} type="button">
      ...
      {chainMismatch ? (
        <div className={`container ${styles.warning}`} role="alert">
          Wallet is on chain {walletChainId}. Reads still show Robinhood Chain, but management actions will ask the wallet to switch to 4663.
        </div>
      ) : null}
                  ...
                  {confirm === "revoke" ? (
                    <div className={styles.confirmBox} role="alert">
                      ...
                      <div>
                        ...
                        <button className="btn btnPrimary" data-busy={busy === `revoke:${automation.key}`} disabled={busy !== null} onClick={() => void submitRevoke(automation)} type="button">
                    ...
                    <div className={styles.confirmBox} role="alert">
                      ...
                      <div>
                        ...
                        <button className="btn btnPrimary" data-busy={busy === `recover:${automation.key}`} disabled={busy !== null || zapSummary?.managementReadsStatus !== "live"} onClick={() => void submitRecovery(automation)} type="button">
                    ...
                    <div className={styles.autoActions}>
                      {automation.state.cancelable && automation.parsed ? (
                        <button className="btn btnGhost" disabled={busy !== null} onClick={() => setConfirmAction({ kind: "revoke", key: automation.key })} type="button">Revoke</button>
                      ) : null}
                      <Link className="btn btnGhost" href={replacementHref(automation)}>Create replacement</Link>
                      <button className="btn btnGhost" disabled={busy !== null || zapSummary?.managementReadsStatus !== "live"} onClick={() => setConfirmAction({ kind: "recover", key: automation.key })} type="button">Recover funds</button>
                    </div>
                  )}
                </article>
              );
