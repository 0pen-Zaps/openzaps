...
describe("Agent Kit Discord campaign migration", () => {
  const campaign = reviewedMarketingCampaign(
    AGENT_KIT_MARKETING_CAMPAIGN_ID,
    "discord",
  );

  it("queues the exact source-reviewed Discord artifact once", () => {
    ...
    expect(migration).not.toMatch(/'x'\s*,/u);
    expect(migration).toContain(campaign.body);
    expect(migration).toContain(`'${campaign.contentHash}'`);
    expect(migration).toContain("'2026-08-03T14:00:00.000Z'::timestamptz");
