...
`0xZAPS` is the ERC-20 paired with aeWETH in the live pool. Ownership alone confers no yield, equity, revenue claim, governance, or protocol access — core workflows are never token-gated. A separately identified seven-day campaign holds 50 tokenized Clanker fee shares and distributes campaign-accounted WETH to wallets that deliberately stake. Its configured harvest path collects those Clanker fees; the site never asks for or spends sponsor-wallet WETH, while the campaign contract can also synchronize WETH sent directly to it. The lottery pot's 20% share of the protocol fee converts to 0xZAPS and accrues to Zap owners as tickets; winner selection is governance-gated and the pot has no owner drain.
