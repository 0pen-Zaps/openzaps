...
`0xZAPS` is the ERC-20 paired with aeWETH in the live pool. Ownership alone confers no yield, equity, revenue claim, governance, or protocol access — core workflows are never token-gated. A separately identified Aug 3–10, 2026 campaign was funded at launch with 50 tokenized Clanker fee shares and is configured to allocate campaign-accounted WETH by time-weighted stake during that fixed window. Its harvest path was configured for those Clanker fees; the site never asks for or spends sponsor-wallet WETH, while the campaign contract can also synchronize WETH sent directly to it. The current principal, phase, and claim deadline are published at `/rewards`. The lottery pot's 20% share of the protocol fee converts to 0xZAPS and accrues to Zap owners as tickets; winner selection is governance-gated and the pot has no owner drain.
