...
- **50% market-buys $HOOKR and permanently bonds it into Hook Blocks** — an
  append-only, exit-less onchain ledger. Bonded HOOKR can never leave.

Every trade on the 0xZAPS pool therefore does two things during the window:
pays stakers in WETH, and permanently locks $HOOKR into the protocol.
...
2. **Claim its WETH.** Every `bond()` first pulls the vault's accrued WETH
...
3. **Market-buy HOOKR.** Unwraps up to `MAX_BOND_WEI` of WETH to native ETH
...
5. **Bond permanently.** Bought HOOKR is recorded as one immutable **Hook
   Block** — `(wethIn, hookrBonded, timestamp)` appended to a public ledger
   with cumulative totals. **No function in the contract can move HOOKR
   out.** Not the sponsor, not governance, nobody. That is the product.
...
- at most `MAX_BOND_WEI` converted per call,
- at most one `bond()` per block (global),
...
| `MAX_BOND_WEI` | 0.05 ETH | ~2 days of the leg's expected flow (~0.023 ETH/day); bounds one sandwich. |
| `MIN_BOND_WEI` | 0.0005 ETH | Dust swaps waste more gas than they bond. |
| `SWEEP_AFTER` | `endAt + 30 days` | A month of permissionless bonding before the escape hatch opens. |
...
- Never "deflationary", never "price support", never "buyback" as a benefit
  claim, never a rate or APY. The mechanism is stated as flows: fees are
  split, one half streams to stakers, the other half is converted to HOOKR
  and locked by construction.
- "Permanently bonded" must always mean the ledger's no-exit construction
  and should link the contract.
