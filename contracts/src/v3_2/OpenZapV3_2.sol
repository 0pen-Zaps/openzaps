...
import {RecurringRelativeIntent} from "../v3_1/libraries/OpenZapV3_1Types.sol";
import {RecurringStackIntent} from "./libraries/OpenZapV3_2Types.sol";
...
/// @title OpenZapV3_2
/// @notice UNAUDITED v3.2 CANDIDATE. A strict SUPERSET of `OpenZapV3_1`: the one-shot, recurring,
///         triggered, and recurring-relative paths are carried over byte-for-byte (same checks, same
///         nonce/series semantics, same 1% fee split 80/20 executor/pot, same isolation and recovery),
///         and ONE new execution type is added:
///
///         5. RECURRING-STACK — `executeRecurringStack(RecurringStackIntent, sig)`: everything
///            `executeRecurringRelative` does (cadence, nonce, authorization, executor pin, fee split,
///            spot-derived per-run floor) PLUS an owner-signed `stackBps` slice of each run's post-fee
///            output that is converted into 0xZAPS and staked into the `LOTTERY_POT` as the OWNER's
///            tickets. Every run of a stacking series is a real market buy of the protocol token.
///
///         WHY: the pot already receives 20% of the 1% protocol fee, but that is the PROTOCOL's slice
///         and no user chooses it. This type makes buying 0xZAPS a first-class, owner-authorized
///         property of an execution — the owner decides how much of their own throughput becomes
///         token demand, and is credited lottery tickets for exactly what they contributed.
///
/// @dev THE CONVERSION REUSES THE POT'S PROVEN SHAPE, NOT A NEW ONE. `ZapLotteryPot.buyZaps` already
///      converts an arbitrary fee asset into 0xZAPS through ONE pinned bounded adapter, measuring the
///      result by BALANCE DELTA rather than trusting the adapter's return value. The stack leg here
///      does the same thing with the same pinned-adapter discipline. Nothing about the deployed pot
///      changes: `notifyContribution` ALREADY credits both tickets and `roundPrize` when the asset it
///      is told about is 0xZAPS itself (see ZapLotteryPot:130), which is precisely what a stack
///      contribution is. This capsule therefore composes with the existing immutable pot instead of
///      requiring a replacement — an app-level concern solved without touching a working core.
///
/// @dev TWO FLOORS, BOTH SPOT-DERIVED. `priceSource`/`maxSlippageBps` floor the RECIPIENT's leg
///      exactly as v3.1 does; `stackPriceSource` floors the 0xZAPS CONVERSION leg. Both are required
///      because they protect against different losses: without the first, the run itself is a bad
///      trade; without the second, a manipulated 0xZAPS pool converts a real slice of the owner's
///      output into dust tickets. Freezing either floor at signing would reintroduce the exact defect
///      v3.1 was built to fix, so neither is frozen.
///
/// @dev THE RECIPIENT'S FLOOR IS SCALED BY THE STACK SLICE, NOT APPLIED BEFORE IT. The slice is a
///      real diversion: the recipient genuinely receives `(10_000 - stackBps)/10_000` of the post-fee
///      output. Checking the unscaled v3.1 floor against the PRE-slice amount would let a run pass
///      its floor and still hand the recipient less than that floor — a silent under-delivery of a
///      bound the owner was shown. The floor is scaled to what the recipient actually gets.
///
/// @dev SLIPPAGE MUST EXCEED THE PROTOCOL FEE. The floor is derived from GROSS spot but compared
///      against a NET (post-fee) amount, so any `maxSlippageBps <= EXEC_FEE_BPS` makes the floor
///      unreachable and bricks every run of the series — a live-observed failure on the v3.1 path,
///      where a 100 bps band silently produced a series that could never execute. v3.2 rejects that
///      band AT SIGNING-CHECK TIME with an explicit `SlippageBelowFee`, turning a mystifying
///      permanent revert into a legible one.
///
...
///      "3.2" so a v3.2 clone's signatures are scoped to these semantics, exactly as "3.1" scoped v3.1.
contract OpenZapV3_2 {
    ...
    /// @notice The protocol lottery pot receiving 20% of every executor fee, and the destination of
    ///         every recurring-stack slice.
    address public immutable LOTTERY_POT;
    /// @notice The protocol token every stack slice is converted into. Pinned at construction — a
    ///         clone can never be pointed at a different "0xZAPS".
    address public immutable ZAPS;
    /// @notice The SINGLE bounded adapter (same `IAdapter` surface as policy steps) used to convert a
    ///         stack slice into 0xZAPS. Pinned at construction, mirroring `ZapLotteryPot.BUY_ADAPTER`:
    ///         no routing discretion is granted to the submitter, the owner, or the intent.
    address public immutable ZAPS_ADAPTER;
    ...
    /// @dev The stack typehash. Field order mirrors `RecurringRelativeIntent` with `stackPriceSource`
    ///      + `stackBps` APPENDED — a distinct type, so a v3.1 relative signature can never be
    ///      replayed as a stacking one (which would divert output the signer never authorized).
    bytes32 private constant RECURRING_STACK_TYPEHASH = keccak256(
    ...
    // Version "3.2": a v3.2 clone is a distinct verifying contract, but the bump scopes signatures to
    // the v3.2 semantics (stacking recurring) explicitly, exactly as "3.1" scoped v3.1.
    bytes32 private constant DOMAIN_VERSION = keccak256("3.2");
    ...
    /// @param stackIn  the slice of post-fee `outAsset` diverted out of the recipient's leg
    /// @param zapsOut  the 0xZAPS actually staked to the pot, measured by balance delta
    /// @param floor    the recipient's spot-derived floor, already scaled by `stackBps`
    event ExecutedRecurringStack(
        uint256 indexed seriesId,
        uint32 run,
        address indexed executor,
        address indexed priceSource,
    ...
    /// @dev `stackBps` is zero (a stacking series that stacks nothing is a `RecurringRelativeIntent`
    ///      and should be signed as one) or `>= 10_000` (which would divert the recipient's entire
    ///      leg). Fail closed rather than silently degrade into another type's semantics.
    error InvalidStackBps();
    /// @dev The floor is derived from GROSS spot but enforced against a NET (post-fee) amount, so a
    ///      band at or below `EXEC_FEE_BPS` can never be cleared and would brick every run of the
    ///      series. Reject it explicitly instead of minting a permanently-reverting authorization.
    error SlippageBelowFee();
    /// @dev `stackPriceSource` must be set exactly when a conversion leg exists: zero iff `outAsset`
    ///      IS 0xZAPS (nothing to convert), non-zero otherwise (a conversion that must be floored).
    ///      Both halves are enforced so an unused source can never be carried along stale.
    error StackSourceMismatch();
    /// @dev The pinned 0xZAPS adapter returned a token that is not 0xZAPS. Mirrors
    ///      `ZapLotteryPot.WrongAdapterOutput` — the conversion leg trusts the pin, not the return.
    error WrongStackOutput(address tokenOut);
    /// @dev The conversion produced less 0xZAPS than the spot-derived floor allows (or none at all),
    ///      so the slice would have become dust tickets. Fail the whole run closed.
    error StackFloorNotMet(uint256 got, uint256 want);
    /// @dev The stack slice rounded down to zero, so the run would pay no token demand while still
    ///      presenting itself as a stacking execution. Fail closed rather than silently no-op.
    error StackSliceUnderflow();
    ...
    constructor(
        ...
        address lotteryPot_,
        address zaps_,
        address zapsAdapter_
    ) {
        if (
            factory_ == address(0) || address(adapters_) == address(0) || address(tokens_) == address(0)
                || address(priceSources_) == address(0) || lotteryPot_ == address(0) || zaps_ == address(0)
                || zapsAdapter_ == address(0)
        ) revert ZeroAddress();
        FACTORY = factory_;
        ADAPTERS = adapters_;
        TOKENS = tokens_;
    ...
    // --------------------------------------------------------------------- //
    // Execution type 5: recurring-stack (relative floor + 0xZAPS staking)    //
    // --------------------------------------------------------------------- //

    /// @dev Per-run working set. This path orients TWO pools and measures THREE balance deltas, which
    ///      overruns Solidity's 16-slot stack, so the context travels in memory instead.
    struct StackRun {
        address inAsset;
        address currency1;
        address stackCurrency1;
        uint256 priceX96;
        uint256 stackPriceX96;
        uint256 preIn;
        uint256 preOut;
        uint32 run;
    }

    /// @notice Execute one run of an owner-signed STACKING series: the recurring-relative path, plus a
    ...
    function executeRecurringStack(RecurringStackIntent calldata intent, bytes calldata sig) external nonReentrant {
        ...
        s.runs = ctx.run;
        s.lastRun = uint64(block.timestamp);
        if (ctx.run == intent.maxRuns) {
            nonceUsed[intent.seriesId] = true; // exhaustion consumes the series id
            emit SeriesFinished(intent.seriesId, ctx.run);
        }
        _verifySignature(_hashRecurringStack(intent), sig);

        // ---- orient BOTH legs and read BOTH spots BEFORE the steps move any pool ----
        // The stack leg often trades the SAME pool the run just traded, so its reference must also be
        // the untouched market mid — not the price our own swap produced.
        (ctx.inAsset, ctx.currency1, ctx.priceX96) = _orient(intent.priceSource, intent.outAsset);
        if (intent.stackPriceSource != address(0)) {
            address stackFunding;
            (stackFunding, ctx.stackCurrency1, ctx.stackPriceX96) = _orient(intent.stackPriceSource, ZAPS);
            // The source must price EXACTLY this run's outAsset against 0xZAPS; a source that buys
            // 0xZAPS with some other currency cannot floor this conversion.
            if (stackFunding != intent.outAsset) revert PairNotValuable(intent.outAsset, stackFunding, ZAPS);
        }

        // ---- run + measure this run's ACTUAL input spent and output produced ----
        ctx.preIn = IERC20(ctx.inAsset).balanceOf(address(this));
        ctx.preOut = IERC20(intent.outAsset).balanceOf(address(this));
        _runSteps();
        // Underflow-reverts (fail closed) if the run somehow ended holding MORE input than it started
        // with — i.e. it did not actually spend the pool's input currency.
        uint256 amountInSpent = ctx.preIn - IERC20(ctx.inAsset).balanceOf(address(this));
        if (amountInSpent == 0) revert NoInputSpent();

        // The v3.1 floor, then SCALED to what the recipient actually receives once the slice is taken.
        // Scaling here — rather than checking the unscaled floor against the pre-slice amount — is what
        // stops a run from clearing its floor while handing the recipient less than that floor.
        uint256 floor = V4PoolMath.mulDiv(
            _relativeFloor(intent.outAsset, ctx.currency1, ctx.priceX96, amountInSpent, intent.maxSlippageBps),
            BPS - intent.stackBps,
            BPS
        );
        _settleStack(intent, ctx, floor);
    }

    /// @dev Every static gate of the stacking path, hoisted so the executor body stays readable. All
    ///      of `executeRecurringRelative`'s checks, plus the three the stack leg adds.
    function _validateStackIntent(RecurringStackIntent calldata intent) private view {
        if (intent.zap != address(this)) revert WrongZap();
        if (intent.chainId != block.chainid) revert WrongChain();
        if (intent.policyHash != policyHash) revert PolicyMismatch();
        if (intent.interval == 0 || intent.maxRuns == 0) revert InvalidSchedule();
        ...
        if (!PRICE_SOURCES.isAllowed(intent.priceSource)) revert PriceSourceNotAllowed(intent.priceSource);
        if (intent.maxSlippageBps >= BPS) revert InvalidSlippage(); // >= 100% would disable the floor
        // The floor is GROSS-derived but NET-enforced, so a band inside the fee can never be cleared.
        // v3.1 accepted such a band and produced a series that reverted forever; v3.2 refuses to mint
        // that authorization at all.
        if (intent.maxSlippageBps <= EXEC_FEE_BPS) revert SlippageBelowFee();
        if (intent.stackBps == 0 || intent.stackBps >= BPS) revert InvalidStackBps();
        // A conversion leg exists exactly when the output is not already the protocol token, so the
        // source must be present in exactly that case — never stale-and-unused, never missing-and-needed.
        if ((intent.outAsset == ZAPS) != (intent.stackPriceSource == address(0))) revert StackSourceMismatch();
        if (intent.stackPriceSource != address(0) && !PRICE_SOURCES.isAllowed(intent.stackPriceSource)) {
            revert PriceSourceNotAllowed(intent.stackPriceSource);
        }
        // Same single-step restriction as the relative path, for the same reason: `amountInSpent` is a
        // net delta and a multi-step policy touching the same currency would misvalue the run.
        if (_steps.length != 1) revert RelativeRequiresSingleStep();
        if (nonceUsed[intent.seriesId]) revert NonceReplay(); // cancelled or exhausted series
    }

    /// @dev Resolve which pool currency funds `wantAsset` on `source`, and read the untouched spot.
    ///      Fails closed when the source cannot value the pair or the market is unreadable.
    function _orient(address source, address wantAsset)
        private
        view
        returns (address inAsset, address currency1, uint256 priceX96)
    {
        IOrientedPriceSource src = IOrientedPriceSource(source);
        address c0 = src.currency0();
        currency1 = src.currency1();
        if (wantAsset == currency1) {
            inAsset = c0; // spending currency0, buying currency1 (e.g. aeWETH -> 0xZAPS)
        } else if (wantAsset == c0) {
            inAsset = currency1; // spending currency1, buying currency0 (e.g. 0xZAPS -> aeWETH)
        } else {
            revert PairNotValuable(wantAsset, c0, currency1);
        }
        priceX96 = src.priceX96(); // reverts (fails closed) if the market is unreadable
    }

    /// @dev Settlement for the stacking path. Identical to `_settleWithExecutorFee` through the 1% fee
    ///      split, then diverts `stackBps` of what remains into 0xZAPS staked to the pot. The
    ///      recipient's floor is enforced against the POST-slice amount. The only value that leaves is
    ///      the measured delta (I-FLOW-4).
    function _settleStack(RecurringStackIntent calldata intent, StackRun memory ctx, uint256 floor) private {
        address outAsset = intent.outAsset;
        uint256 out = IERC20(outAsset).balanceOf(address(this)) - ctx.preOut; // underflow-reverts if no gain
        uint256 fee = (out * EXEC_FEE_BPS) / BPS;
        uint256 executorFee = (fee * EXECUTOR_SHARE_BPS) / BPS;
        uint256 potFee = fee - executorFee;
        uint256 postFee = out - fee;

        uint256 stackIn = (postFee * intent.stackBps) / BPS;
        // A slice that rounds to zero would present as a stacking run while buying no token at all.
        if (stackIn == 0) revert StackSliceUnderflow();
        uint256 net = postFee - stackIn;
        if (net < floor) revert MinOutNotMet(); // floor is NET of BOTH the fee and the slice (I-FLOW-2)

        if (executorFee != 0) outAsset.safeTransfer(msg.sender, executorFee);
        if (potFee != 0) {
            outAsset.safeTransfer(LOTTERY_POT, potFee);
            // Credit lottery tickets to the fee payer (this zap's owner). The pot only records — it can
            // never pull. A revert here fails the run closed rather than dropping the credit.
            ILotteryPot(LOTTERY_POT).notifyContribution(owner, outAsset, potFee);
        }

        uint256 zapsOut = _stakeStack(outAsset, stackIn, ctx, intent.maxSlippageBps);
        outAsset.safeTransfer(recipient, net);

        emit ExecutedRecurringStack(
            intent.seriesId,
            ctx.run,
            msg.sender,
            intent.priceSource,
    ...
    /// @dev Convert the slice into 0xZAPS through the PINNED adapter and stake it to the pot as the
    ///      owner's tickets. Output is measured by BALANCE DELTA — the adapter's return value is never
    ///      trusted for accounting, exactly as in `ZapLotteryPot.buyZaps` — and floored at the owner's
    ///      slippage band below the pre-run spot, reusing the SAME `_relativeFloor` math the recipient
    ///      leg uses. When `outAsset` IS 0xZAPS there is no market leg: the slice is already the token.
    function _stakeStack(address outAsset, uint256 stackIn, StackRun memory ctx, uint32 maxSlippageBps)
        private
        returns (uint256 zapsOut)
    {
        ...
        if (outAsset == ZAPS) {
            ...
            outAsset.approveExact(ZAPS_ADAPTER, stackIn);
            (address tokenOut,) = IAdapter(ZAPS_ADAPTER).execute(outAsset, stackIn, "");
            outAsset.approveExact(ZAPS_ADAPTER, 0);
            if (tokenOut != ZAPS) revert WrongStackOutput(tokenOut);
            zapsOut = IERC20(ZAPS).balanceOf(address(this)) - preZaps; // underflow-reverts on loss

            uint256 want = _relativeFloor(ZAPS, ctx.stackCurrency1, ctx.stackPriceX96, stackIn, maxSlippageBps);
            if (zapsOut < want || zapsOut == 0) revert StackFloorNotMet(zapsOut, want);
        }
        ZAPS.safeTransfer(LOTTERY_POT, zapsOut);
        // The pot credits the OWNER tickets for exactly this contribution and — because the asset IS
        // 0xZAPS — also rolls it into the round's prize (ZapLotteryPot:130). Record-only: the pot can
        // never pull. A revert here fails the run closed rather than dropping the credit.
        ILotteryPot(LOTTERY_POT).notifyContribution(owner, ZAPS, zapsOut);
    }
    ...
    function hashRecurringStackIntent(RecurringStackIntent calldata intent) public view returns (bytes32) {
        return _hashRecurringStack(intent);
    }

    function _hashRecurringStack(RecurringStackIntent calldata intent) private view returns (bytes32) {
        bytes32 structHash = keccak256(
            abi.encode(
